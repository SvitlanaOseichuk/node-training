...
async function getBook(req, res, next) {
    ...
    try {
        ...
        // //  перевірка чи належить айді книги саме цьому користувачу
        // if (book.owner.toString() !== req.user.id) {
        //     // return req.status(403).send({message: "Book is not allowed"}); 
        //     return req.status(404).send({message: "Book not found"});
       
        // }

        //2.............

        const book =await Book.findOne({id: id, owner: req.user.id})
